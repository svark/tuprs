...
fn as_glob_match(inpg: &InputGlobType) -> String {
    match inpg {
        InputGlobType::Deglob(e) => e.group(1)
            .and_then(|x| x.to_str())
            .map(|x| x.to_string())
...
impl DecodeInputPaths for Vec<PathExpr> {
    fn decode(&self, tupcwd: &Path, taginfo: &OutputTagInfo) -> Vec<InputGlobType> {
        let excludeindices: Vec<_> = self.iter().enumerate().filter(|pi| {
            matches!(&pi.1, &PathExpr::ExcludePattern(_))
        }).map(|ref pi| pi.0).collect();
...
impl DecodeInputPaths for Statement {
    fn decode(&self, tupcwd: &Path, taginfo: &OutputTagInfo) -> Vec<InputGlobType> {
        if let Statement::Rule(Link { source: s, .. }) = self {
            let inputs = s.primary.decode(tupcwd, taginfo);
            inputs
        } else {
            Vec::new()
        }
...
impl DecodeInputPlaceHolders for PathExpr {
    fn decode_input_place_holders(
        ...
        let frep = |inp: &Vec<&InputGlobType>, d: &str| {
            ...
            decoded_str = grpre.unwrap().replace(decoded_str.as_str(), |caps: &Captures| {
                 namedgroupitems(&caps[1]).join(" ")
            }).to_string();

            let per_cap_b_re = Regex::new(r"%([0-9]*)B");
            if per_cap_b_re.unwrap().captures(decoded_str.as_str()).is_some() {
                let fnameswoe = nongroupsprocessed(&|f|
                    as_path(f).file_stem().and_then(|x| x.to_str()).unwrap_or("").to_string());
            ...
            if let Some(&i) = inp.first() {
                ...
                decoded_str = decoded_str
                    ...
                    .replace("%e", path.extension().and_then(|x| x.to_str()).unwrap_or(""))
                    .replace(
                        "%d",
                        path.parent()
                            .and_then(|x|
...
fn replace_decoded_str(decoded_str: &String, file_names: Vec<String>, perc_b_re: Result<Regex, Error>) -> String{
    perc_b_re.unwrap().replace(decoded_str.as_str(), |caps: &Captures| {
         caps[1].parse::<usize>().map(|i| file_names[i].clone()).unwrap_or("".to_string())
    }).to_string()
}
...
impl DecodeOrderOnlyInputPlaceHolders for PathExpr {
    ...
    {
        let allinputs: Vec<_> =
            orderonlyinputs.into_iter().map(|&x| x.to_string()).collect();
...
impl DecodeOutputPlaceHolders for PathExpr {
    fn decode_output_place_holders(&self, outputs: &Vec<&OutputType>) -> Self {
        let alloutputs: Vec<_> =
            outputs.into_iter().map(|x| x.to_str().unwrap_or("").to_string()).collect();
        let space_separated_outputs = alloutputs.join(" ");
        let frep = |d: &str| {
            d.replace(
                "%o",
                space_separated_outputs.as_str(),
            )
                .replace(
                    "%O",
                    /*
...
fn primary_paths(tupcwd: &Path, tgt: &Target) -> Vec<PathBuf> {
    tgt.primary
        ...
        .map(|x| tupcwd.join(x.to_vec().cat())).collect()
}
...
impl DecodeInputPlaceHolders for Target {
    fn decode_input_place_holders(&self, input: &Vec<&InputGlobType>) -> Target {
        ...
        let newsecondary = self
            .secondary
            .decode_input_place_holders(input);
        Target {
            primary: newprimary,
...
impl DecodeOrderOnlyInputPlaceHolders for Target {
    fn decode_order_only_input_place_holders(&self, orderonlyinputs: &Vec<&InputGlobType>) -> Self {
        let newprimary = self.primary.decode_order_only_input_place_holders(orderonlyinputs);
        let newsecondary = self
            .secondary
...
impl DecodeOrderOnlyInputPlaceHolders for RuleFormula {
    fn decode_order_only_input_place_holders(
        &self,
        inputs: &Vec<&InputGlobType>,
    ) -> RuleFormula {
        RuleFormula {
...
pub fn deglobrule(tupfile: &Path, stmt: &Statement, taginfo: &OutputTagInfo) -> (Vec<Statement>, OutputTagInfo) {
    ...
    if let Statement::Rule(Link { source: s, target: t, rule_formula: r, pos }) = stmt {
        ...
        if for_each {
            for input in inpdec {
                let vis = vec![&input];
                deglobbed.push(get_deglobbed_rule(tupcwd, &t, r,
                                                  pos, &secondaryinputsasref,
                                                  sinputs, &vis));
                if let Some(&Statement::Rule(Link{source:_, ref target, .. })) = deglobbed.last() {
                    updatetags(&target,
                               &mut output);
                }
            }
...
fn get_deglobbed_rule(tupcwd: &Path, t: &&Target, r: &RuleFormula, pos: &(u32, usize),
                      secondaryinputsasref: &Vec<&InputGlobType>,
                      sinputs: &mut Vec<PathExpr>, vis: &Vec<&InputGlobType>) -> Statement {
    ...
    tc.secondary = tc.secondary.decode_output_place_holders(&primary_output_paths);

    //todo: is this needed: tc.group.map(|x| x.decode_input_place_holders(&vis));

    // now track the outputs that fall in a group or bin
    let rfc: RuleFormula = r.decode_input_place_holders(&vis)
        .decode_order_only_input_place_holders(&secondaryinputsasref)
